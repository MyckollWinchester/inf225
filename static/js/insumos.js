...
async function getExistencias() {
  ...
  favButtons.forEach(async favButton => {
    const card = favButton.parentElement

    const cardLink = card.children[3]
    const link = cardLink.href

    const existencia = await getExistencia(link);
    if (existencia) {
...
searchForm.addEventListener('submit', async e => {
  ...
  favButtons.forEach(favButton => {
    favButton.addEventListener('click', async () => {
      ...
      const imageDiv = card.children[0].children[0]
      const image = imageDiv.src

      const cardName = card.children[1]
      const name = cardName.textContent

      const cardPrice = card.children[2]
      ...
      if (favButton.classList.contains('fav-button--active')) {
        ...
        await fetch('http://localhost:8000/desmarcar-insumo/', {
          method: 'POST',
          cors: 'no-cors',
          headers: {
        ...
        await fetch('http://localhost:8000/marcar-insumo/', {
          method: 'POST',
          cors: 'no-cors',
          headers: {
            'Content-Type': 'application/json'
